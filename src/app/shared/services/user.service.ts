...
export class UserService {
  ...
  public commentsArray: Observable<
    Comment[]
  ...
  public reviewsArray: Observable<
    Review[]
  ...
  public friendArray: Observable<Friend[]> = this.friendSubject.asObservable();
  ...
  signup(newUser: NewUser) {
    ...
    this.httpClient.post(url, newUser).subscribe((data: NewUser) => {
  ...
  async getComments(userid: number, page: number) {
    const url =
      "http://localhost:8080/comments/user=" + userid + "/page=" + page;
    ...
    this.commentSubscription = await this.httpClient
      .get(url)
  ...
  async getReviews(userid: number, page: number) {
    const url =
      "http://localhost:8080/reviews/user=" + userid + "/page=" + page;
    ...
    this.reviewSubscription = await this.httpClient
      .get(url)
  ...
  async getFriends(userid: number) {
    ...
    this.friendSubscription = await this.httpClient
      .get(url)
